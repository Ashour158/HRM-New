...
|---|---|---|---|---|---|---|
| 1 | Product completeness / business logic | 4 | **3** | 8 | 4.8 | Broader surface, but 4 headline features (I-9/E-Verify, works-council, country-rule-set/statutory-leave-type lifecycle, HR-AI kill switch) are unreachable/no-op; "0 stubs" no longer holds — these are worse than stubs because they're *hidden* |
...
| 5 | Architecture / integration | 5 | **4** | 7 | 5.6 | Saga idempotency gap, including the Kafka consumer that swallows handler errors without rethrow and thereby enables the redelivery scenario that triggers it — both part of the H-2 finding, see `docs/GAP-ANALYSIS-2026-07-19.md` |
...
The 2026-06-28 score was earned by closing every open P0/P1 from the 2026-06-25 baseline and having each fix independently re-audited before counting it. This re-score found **6 new CONFIRMED CRITICAL findings and 7 new CONFIRMED HIGH findings (H-1–H-7)** — every one of these 13 surviving a dedicated adversarial verification pass with direct evidence re-derived from `origin/main`, not taken on the specialist's word — plus **one additional real, evidence-backed high finding (H-8)** that was not put through that same dedicated verification pass (see `docs/GAP-ANALYSIS-2026-07-19.md` §1 for the distinction). Per the same "no Ready with any open P0" rule the 2026-06-25 and 2026-06-28 audits both applied, this re-score cannot be READY:
