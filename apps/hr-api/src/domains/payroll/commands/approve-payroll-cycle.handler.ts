...
@CommandHandler('ApprovePayrollCycle')
@Injectable()
export class ApprovePayrollCycleHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { payrollCycleId: Uuid; approvedBy: Uuid };
    const pc = await this.repo.findById(payload.payrollCycleId);
    if (!pc) throw new Error('Payroll cycle not found');
    pc.approve(payload.approvedBy, command.correlationId);
