...
@CommandHandler('ExplainPayrollResultLine')
@Injectable()
export class ExplainPayrollResultLineHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { payrollResultLineId: Uuid; explanation: string };
    const line = await this.repo.findById(payload.payrollResultLineId);
    if (!line) throw new Error('Payroll result line not found');
    line.explain(payload.explanation, command.correlationId);
