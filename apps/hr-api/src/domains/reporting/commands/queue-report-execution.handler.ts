...
export interface QueueReportExecutionPayload {
  reportExecutionId: string;
}

@Injectable()
@CommandHandler('QueueReportExecution')
export class QueueReportExecutionHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as QueueReportExecutionPayload;
    ...
    entity.queue(command.correlationId);
