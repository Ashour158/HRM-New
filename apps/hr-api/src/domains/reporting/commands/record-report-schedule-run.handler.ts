...
export interface RecordReportScheduleRunPayload {
  reportScheduleId: string;
  ranAt: Date;
}

@Injectable()
@CommandHandler('RecordReportScheduleRun')
export class RecordReportScheduleRunHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as RecordReportScheduleRunPayload;
    ...
    entity.recordRun(payload.ranAt, command.correlationId);
    await this.repo.save(entity);
    await this.publisher.publishFromAggregate(entity);
    return {
      success: true,
      data: { reportScheduleId: entity.id.value, status: entity.status, nextRunAt: entity.nextRunAt, lastRunAt: entity.lastRunAt },
