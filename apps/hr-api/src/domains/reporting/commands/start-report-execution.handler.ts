...
export interface StartReportExecutionPayload {
  reportExecutionId: string;
}

@Injectable()
@CommandHandler('StartReportExecution')
export class StartReportExecutionHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as StartReportExecutionPayload;
