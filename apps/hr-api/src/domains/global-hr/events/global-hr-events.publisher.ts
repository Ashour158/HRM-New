...
/**
 * Domain-specific event publisher for Global HR aggregates.
 */
@Injectable()
export class GlobalHrEventsPublisher {
  private readonly logger = new Logger(GlobalHrEventsPublisher.name);
  ...
  async publishUncommitted(aggregate: AggregateRoot, tenantId: Uuid, correlationId: Uuid): Promise<void> {
    ...
    for (const event of events) {
      ...
      this.logger.log({
        type: 'GLOBAL_HR_EVENT_PUBLISHED',
