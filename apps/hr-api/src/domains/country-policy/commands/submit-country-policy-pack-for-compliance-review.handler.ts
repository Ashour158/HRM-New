...
export interface SubmitCountryPolicyPackForComplianceReviewPayload {
  packId: string;
}

/**
 * Handler for the SubmitForComplianceReview command.
 */
@Injectable()
@CommandHandler('SubmitForComplianceReview')
export class SubmitCountryPolicyPackForComplianceReviewHandler implements ICommandHandler {
  readonly commandName = 'SubmitForComplianceReview';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as SubmitCountryPolicyPackForComplianceReviewPayload;
    const pack = await this.repo.findById(new Uuid(payload.packId));
    if (!pack) {
      throw new ValidationError('Country policy pack not found');
    }

    pack.submitForComplianceReview(command.correlationId);
