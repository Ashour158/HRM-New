...
export interface SubmitCountryPolicyPackForBenefitsReviewPayload {
  packId: string;
}

/**
 * Handler for the SubmitForBenefitsReview command.
 */
@Injectable()
@CommandHandler('SubmitForBenefitsReview')
export class SubmitCountryPolicyPackForBenefitsReviewHandler implements ICommandHandler {
  readonly commandName = 'SubmitForBenefitsReview';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as SubmitCountryPolicyPackForBenefitsReviewPayload;
    const pack = await this.repo.findById(new Uuid(payload.packId));
    if (!pack) {
      throw new ValidationError('Country policy pack not found');
    }

    pack.submitForBenefitsReview(command.correlationId);
