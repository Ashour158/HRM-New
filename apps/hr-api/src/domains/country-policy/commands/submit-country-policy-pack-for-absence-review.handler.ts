...
export interface SubmitCountryPolicyPackForAbsenceReviewPayload {
  packId: string;
}

/**
 * Handler for the SubmitForAbsenceReview command.
 */
@Injectable()
@CommandHandler('SubmitForAbsenceReview')
export class SubmitCountryPolicyPackForAbsenceReviewHandler implements ICommandHandler {
  readonly commandName = 'SubmitForAbsenceReview';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as SubmitCountryPolicyPackForAbsenceReviewPayload;
    const pack = await this.repo.findById(new Uuid(payload.packId));
    if (!pack) {
      throw new ValidationError('Country policy pack not found');
    }

    pack.submitForAbsenceReview(command.correlationId);
