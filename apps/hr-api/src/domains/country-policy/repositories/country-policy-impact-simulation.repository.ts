...
export class CountryPolicyImpactSimulationRepository {
  ...
  async save(entity: CountryPolicyImpactSimulation): Promise<void> {
    ...
    const row = {
      ...
      impacted_workers: JSON.stringify(entity.impactedWorkers ?? []),
      impacted_payroll_runs: JSON.stringify(entity.impactedPayrollRuns ?? []),
      impacted_tax_assignments: JSON.stringify(entity.impactedTaxAssignments ?? []),
      impacted_leave_balances: JSON.stringify(entity.impactedLeaveBalances ?? []),
      impacted_benefits: JSON.stringify(entity.impactedBenefits ?? []),
