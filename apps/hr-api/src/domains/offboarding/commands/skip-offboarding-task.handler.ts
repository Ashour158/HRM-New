...
export interface SkipOffboardingTaskCommandPayload {
  taskId: UuidInput;
}

/**
 * Handler for the SkipOffboardingTask command.
 *
 * Transitions a task to SKIPPED (terminal).
 */
@Injectable()
@CommandHandler('SkipOffboardingTask')
export class SkipOffboardingTaskHandler implements ICommandHandler {
  readonly commandName = 'SkipOffboardingTask';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as SkipOffboardingTaskCommandPayload;
    ...
    return {
      ...
      eventsEmitted: ['OffboardingTaskSkipped'],
