...
export interface ResolveHrAiKillSwitchPayload {
  hrAiKillSwitchId: string;
  resolution: string;
}

@Injectable()
@CommandHandler('ResolveHrAiKillSwitch')
export class ResolveHrAiKillSwitchHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as ResolveHrAiKillSwitchPayload;
    ...
    entity.resolve(command.correlationId, payload.resolution);
