...
export interface InvestigateHrAiKillSwitchPayload {
  hrAiKillSwitchId: string;
}

@Injectable()
@CommandHandler('InvestigateHrAiKillSwitch')
export class InvestigateHrAiKillSwitchHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as InvestigateHrAiKillSwitchPayload;
    ...
    entity.investigate(command.correlationId);
