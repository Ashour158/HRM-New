...
export interface RejectHrAiUseCasePayload {
  hrAiUseCaseId: string;
}

@Injectable()
@CommandHandler('RejectHrAiUseCase')
export class RejectHrAiUseCaseHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as RejectHrAiUseCasePayload;
    ...
    entity.reject(command.correlationId);
