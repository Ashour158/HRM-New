...
export interface ApproveHrAiUseCasePayload {
  hrAiUseCaseId: string;
}

@Injectable()
@CommandHandler('ApproveHrAiUseCase')
export class ApproveHrAiUseCaseHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as ApproveHrAiUseCasePayload;
    ...
    entity.approve(command.correlationId);
