...
export interface ReviewHrAiUseCasePayload {
  hrAiUseCaseId: string;
}

@Injectable()
@CommandHandler('ReviewHrAiUseCase')
export class ReviewHrAiUseCaseHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as ReviewHrAiUseCasePayload;
