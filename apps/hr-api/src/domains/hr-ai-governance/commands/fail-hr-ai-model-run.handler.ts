...
export interface FailHrAiModelRunPayload {
  hrAiModelRunId: string;
  reason: string;
}

@Injectable()
@CommandHandler('FailHrAiModelRun')
export class FailHrAiModelRunHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as FailHrAiModelRunPayload;
