...
/**
 * Domain-specific event publisher for Onboarding aggregates.
 *
...
export class OnboardingEventsPublisher {
  private readonly logger = new Logger(OnboardingEventsPublisher.name);
  ...
  async publishUncommitted(aggregate: AggregateRoot, tenantId: Uuid, correlationId: Uuid): Promise<void> {
    ...
    for (const event of events) {
      ...
      this.logger.log({
        type: 'ONBOARDING_EVENT_PUBLISHED',
