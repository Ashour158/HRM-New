...
/**
 * Domain-specific event publisher for Recruiting aggregates.
 *
...
export class RecruitingEventsPublisher {
  private readonly logger = new Logger(RecruitingEventsPublisher.name);
  ...
  async publishUncommitted(aggregate: AggregateRoot, tenantId: Uuid, correlationId: Uuid): Promise<void> {
    ...
    for (const event of events) {
      ...
      this.logger.log({
        type: 'RECRUITING_EVENT_PUBLISHED',
