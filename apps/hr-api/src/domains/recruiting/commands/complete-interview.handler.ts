...
export interface CompleteInterviewCommandPayload {
  interviewId: Uuid;
}

/**
 * Handler for the CompleteInterview command.
 *
 * Transitions an interview plan from IN_PROGRESS to COMPLETED.
 */
@Injectable()
@CommandHandler('CompleteInterview')
export class CompleteInterviewHandler implements ICommandHandler {
  readonly commandName = 'CompleteInterview';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as CompleteInterviewCommandPayload;
    ...
    return {
      ...
      eventsEmitted: ['InterviewPlanCompleted'],
