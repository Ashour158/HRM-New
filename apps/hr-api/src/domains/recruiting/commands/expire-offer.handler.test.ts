...
describe('ExpireOfferHandler', () => {
  ...
  const offerId = new Uuid('00000000-0000-0000-0000-000000000602');
  ...
  it('transitions a SENT offer to EXPIRED', async () => {
  ...
  it('rejects the transition when the offer is not SENT', async () => {
    vi.mocked(offerRepo.findById).mockResolvedValue(offerInState('DRAFT'));

    await expect(handler.handle(command())).rejects.toThrow('Offer can only expire from SENT state');
