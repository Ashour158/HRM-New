...
export interface PublishJobRequisitionCommandPayload {
  requisitionId: Uuid;
}

/**
 * Handler for the PublishJobRequisition command.
 *
 * Transitions a requisition from APPROVED to PUBLISHED.
 */
@Injectable()
@CommandHandler('PublishJobRequisition')
export class PublishJobRequisitionHandler implements ICommandHandler {
  readonly commandName = 'PublishJobRequisition';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as PublishJobRequisitionCommandPayload;
    ...
    requisition.publish(command.correlationId);
    ...
    return {
      ...
      eventsEmitted: ['JobRequisitionPublished'],
