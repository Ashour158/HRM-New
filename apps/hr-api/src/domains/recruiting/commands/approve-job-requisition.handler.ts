...
export interface ApproveJobRequisitionCommandPayload {
  requisitionId: Uuid;
}

/**
 * Handler for the ApproveJobRequisition command.
 *
 * Transitions a requisition from PENDING_APPROVAL to APPROVED.
 */
@Injectable()
@CommandHandler('ApproveJobRequisition')
export class ApproveJobRequisitionHandler implements ICommandHandler {
  readonly commandName = 'ApproveJobRequisition';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as ApproveJobRequisitionCommandPayload;
    ...
    requisition.approve(command.correlationId);
    ...
    return {
      ...
      eventsEmitted: ['JobRequisitionApproved'],
