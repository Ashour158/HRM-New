 ...
 *
 * Transitions an interview plan from SCHEDULED to IN_PROGRESS. Without
 * this handler, CompleteInterview (which requires IN_PROGRESS) could
 * never succeed -- a scheduled interview had no way to be marked started
 * (HCM-P0-7).
 */
@Injectable()
@CommandHandler('StartInterview')
export class StartInterviewHandler implements ICommandHandler {
  readonly commandName = 'StartInterview';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as StartInterviewCommandPayload;
