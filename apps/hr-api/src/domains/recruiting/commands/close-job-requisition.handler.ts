...
export interface CloseJobRequisitionCommandPayload {
  requisitionId: Uuid;
  reason: string;
}

/**
 * Handler for the CloseJobRequisition command.
 *
 * Moves a requisition to the CLOSED terminal state.
 */
@Injectable()
@CommandHandler('CloseJobRequisition')
export class CloseJobRequisitionHandler implements ICommandHandler {
  readonly commandName = 'CloseJobRequisition';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as CloseJobRequisitionCommandPayload;
    ...
    requisition.close(payload.reason, command.correlationId);
    ...
    return {
      ...
      eventsEmitted: ['JobRequisitionClosed'],
