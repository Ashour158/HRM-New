...
export interface FillJobRequisitionCommandPayload {
  requisitionId: Uuid;
}

/**
 * Handler for the FillJobRequisition command.
 *
 * Transitions a job requisition from OPEN to FILLED.
 */
@Injectable()
@CommandHandler('FillJobRequisition')
export class FillJobRequisitionHandler implements ICommandHandler {
  readonly commandName = 'FillJobRequisition';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as FillJobRequisitionCommandPayload;
