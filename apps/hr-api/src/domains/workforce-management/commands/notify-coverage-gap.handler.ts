...
@CommandHandler('NotifyCoverageGap')
@Injectable()
export class NotifyCoverageGapHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { coverageGapId: Uuid };
    const ar = await this.repo.findById(payload.coverageGapId);
    if (!ar) throw new Error('Coverage gap not found');
    ar.notify(command.correlationId);
