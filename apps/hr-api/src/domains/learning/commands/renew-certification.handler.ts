...
@CommandHandler('RenewCertification')
@Injectable()
export class RenewCertificationHandler {
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { certificationId: Uuid; newExpiryDate: Date };
    const ar = await this.repo.findById(payload.certificationId);
    if (!ar) throw new Error('Certification not found');
    ar.renew(payload.newExpiryDate, command.correlationId);
