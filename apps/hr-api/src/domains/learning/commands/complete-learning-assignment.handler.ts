...
@CommandHandler('CompleteLearningAssignment')
@Injectable()
export class CompleteLearningAssignmentHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { learningAssignmentId: Uuid; score: number };
    const ar = await this.repo.findById(payload.learningAssignmentId);
    if (!ar) throw new Error('Learning assignment not found');
    ar.complete(payload.score, command.correlationId);
