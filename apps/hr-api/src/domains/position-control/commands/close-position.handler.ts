...
/**
 * Handler for the ClosePosition command.
 */
@Injectable()
@CommandHandler('ClosePosition')
export class ClosePositionHandler implements ICommandHandler {
  readonly commandName = 'ClosePosition';
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as ClosePositionCommandPayload;
    const position = await this.positionRepo.findById(new Uuid(payload.positionId));
    if (!position) {
      throw new NotFoundException('Position not found');
    }

    position.close(command.correlationId);
