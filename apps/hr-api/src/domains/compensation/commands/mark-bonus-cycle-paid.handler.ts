...
/**
 * Command handler for marking a BonusCycle's payout as paid.
 * APPROVED → PAID.
 */
@Injectable()
@CommandHandler('MarkBonusCyclePaid')
export class MarkBonusCyclePaidHandler implements ICommandHandler {
  commandName = 'MarkBonusCyclePaid' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    cycle.markPaid(command.correlationId);
