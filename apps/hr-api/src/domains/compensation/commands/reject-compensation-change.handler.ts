...
/**
 * Command handler for rejecting a CompensationChange (PENDING_APPROVAL -> REJECTED).
 */
@Injectable()
@CommandHandler('RejectCompensationChange')
export class RejectCompensationChangeHandler implements ICommandHandler {
  commandName = 'RejectCompensationChange' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    change.reject(command.correlationId);
