...
/**
 * Command handler for closing a CompensationPlan.
 * ACTIVE or SUSPENDED → CLOSED (terminal).
 */
@Injectable()
@CommandHandler('CloseCompensationPlan')
export class CloseCompensationPlanHandler implements ICommandHandler {
  commandName = 'CloseCompensationPlan' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    if (!plan) throw new Error('CompensationPlan not found');

    plan.close(command.correlationId);
