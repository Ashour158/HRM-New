...
/**
 * Command handler for starting the vesting clock on an EquityGrant.
 * GRANTED → VESTING.
 */
@Injectable()
@CommandHandler('StartEquityGrantVesting')
export class StartEquityGrantVestingHandler implements ICommandHandler {
  commandName = 'StartEquityGrantVesting' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    grant.startVesting(command.correlationId);
