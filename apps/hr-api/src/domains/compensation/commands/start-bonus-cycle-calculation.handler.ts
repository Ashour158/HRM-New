...
/**
 * Command handler for starting bonus payout calculation for a BonusCycle.
 * ACTIVE → CALCULATION.
 */
@Injectable()
@CommandHandler('StartBonusCycleCalculation')
export class StartBonusCycleCalculationHandler implements ICommandHandler {
  commandName = 'StartBonusCycleCalculation' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    cycle.startCalculation(command.correlationId);
