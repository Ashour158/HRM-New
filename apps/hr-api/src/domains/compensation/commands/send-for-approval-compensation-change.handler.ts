...
/**
 * Command handler for moving a CompensationChange to pending approval
 * (SUBMITTED -> PENDING_APPROVAL). Required for ApproveCompensationChange
 * (which requires PENDING_APPROVAL) to ever be reachable.
 */
@Injectable()
@CommandHandler('SendForApprovalCompensationChange')
export class SendForApprovalCompensationChangeHandler implements ICommandHandler {
  commandName = 'SendForApprovalCompensationChange' as const;
