...
export class ExerciseEquityGrantHandler implements ICommandHandler {
  commandName = 'ExerciseEquityGrant' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    grant.exercise(payload.units, command.correlationId);
    ...
    return {
      success: true,
      data: { grantId: grant.id.value, status: grant.status, exercisedUnits: grant.exercisedUnits },
