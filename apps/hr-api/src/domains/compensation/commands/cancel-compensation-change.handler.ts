...
/**
 * Command handler for cancelling a CompensationChange
 * (DRAFT, SUBMITTED, or PENDING_APPROVAL -> CANCELLED).
 */
@Injectable()
@CommandHandler('CancelCompensationChange')
export class CancelCompensationChangeHandler implements ICommandHandler {
  commandName = 'CancelCompensationChange' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    change.cancel(command.correlationId);
