...
import type { PayScaleStep } from '../aggregates/pay-scale.aggregate.js';
...
export class RevisePayScaleHandler implements ICommandHandler {
  commandName = 'RevisePayScale' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { scaleId: Uuid; steps: PayScaleStep[] };
    const scale = await this.repo.findById(payload.scaleId);
    if (!scale) throw new Error('PayScale not found');

    scale.revise(payload.steps, command.correlationId);
