...
export interface PublishPayGapReportPayload {
  payGapReportId: string;
}

@Injectable()
@CommandHandler('PublishPayGapReport')
export class PublishPayGapReportHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as PublishPayGapReportPayload;
