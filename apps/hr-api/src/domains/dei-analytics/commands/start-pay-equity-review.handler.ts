...
export interface StartPayEquityReviewPayload {
  payEquityReviewId: string;
}

@Injectable()
@CommandHandler('StartPayEquityReview')
export class StartPayEquityReviewHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as StartPayEquityReviewPayload;
