...
export interface PublishDeiReportPayload {
  deiReportId: string;
}

@Injectable()
@CommandHandler('PublishDeiReport')
export class PublishDeiReportHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as PublishDeiReportPayload;
