  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    return {
      ...
      data: {
        ...
        coverageStartDate: enrollment.effectiveDate.toISOString().slice(0, 10),
        dependentCount: enrollment.dependents.length,
        dependents: enrollment.dependents.map((dependent) => ({
