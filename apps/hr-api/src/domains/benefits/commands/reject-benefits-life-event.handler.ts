...
@Injectable()
@CommandHandler('RejectBenefitsLifeEvent')
export class RejectBenefitsLifeEventHandler implements ICommandHandler {
  commandName = 'RejectBenefitsLifeEvent' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { lifeEventId: Uuid; rejectedBy?: Uuid; reason?: string };
    const event = await this.repo.findById(payload.lifeEventId);
    if (!event) throw new Error('BenefitsLifeEvent not found');

    event.reject(command.correlationId);
