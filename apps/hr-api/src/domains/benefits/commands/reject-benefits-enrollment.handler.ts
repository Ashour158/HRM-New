...
@Injectable()
@CommandHandler('RejectBenefitsEnrollment')
export class RejectBenefitsEnrollmentHandler implements ICommandHandler {
  commandName = 'RejectBenefitsEnrollment' as const;
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { enrollmentId: Uuid; rejectedBy?: Uuid; reason?: string };
    ...
    enrollment.reject(command.correlationId);
    ...
    return {
      ...
      data: {
        ...
        rejectedBy: payload.rejectedBy?.value,
