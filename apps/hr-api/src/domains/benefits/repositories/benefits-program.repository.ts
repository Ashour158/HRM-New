...
export class BenefitsProgramRepository extends BaseRepository<'benefits_programs', BenefitsProgram> {
  ...
  async findByTenant(tenantId: Uuid): Promise<BenefitsProgram[]> {
    const rows = await this.db
      ...
      .execute();
    return rows.map((r: any) => this.toAggregate(r as unknown as Database['benefits_programs']));
  }
  ...
  async save(entity: BenefitsProgram): Promise<void> {
    ...
    if (existing) {
      await this.update(entity.id, row as unknown as Updateable<Database['benefits_programs']>, { expectedVersion: entity.loadedVersion });
  ...
  private toRow(entity: BenefitsProgram): Record<string, unknown> {
    return {
      ...
      monthly_premium: entity.monthlyPremium,
