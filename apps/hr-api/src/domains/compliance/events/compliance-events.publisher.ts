...
/**
 * Domain-specific event publisher for Compliance aggregates.
 *
 * Reads uncommitted domain events from an aggregate, maps them to canonical
 * {@link HrEventEnvelope} instances, and publishes them to the appropriate topic.
 */
@Injectable()
export class ComplianceEventsPublisher {
  private readonly logger = new Logger(ComplianceEventsPublisher.name);
  ...
  async publishUncommitted(aggregate: AggregateRoot, tenantId: Uuid, correlationId: Uuid): Promise<void> {
    ...
    for (const event of events) {
      ...
      this.logger.log({
        type: 'COMPLIANCE_EVENT_PUBLISHED',
