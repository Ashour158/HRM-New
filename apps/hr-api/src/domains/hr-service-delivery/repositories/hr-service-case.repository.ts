...
export class HrServiceCaseRepository extends BaseRepository<'hr_service_cases', HrServiceCase> {
  ...
  private toAggregate(row: Database['hr_service_cases']): HrServiceCase {
    return new HrServiceCase({
      ...
      catalogItemId: row.catalog_item_id ? new Uuid(row.catalog_item_id) : undefined,
      ...
      escalatedBy: row.escalated_by ? new Uuid(row.escalated_by) : undefined,
  ...
  private toRow(entity: HrServiceCase): Record<string, unknown> {
    return {
      ...
      catalog_item_id: entity.catalogItemId?.value ?? null,
      owner_group: entity.ownerGroup ?? null,
      escalation_reason: entity.escalationReason ?? null,
      escalated_at: entity.escalatedAt ?? null,
      escalated_by: entity.escalatedBy?.value ?? null,
