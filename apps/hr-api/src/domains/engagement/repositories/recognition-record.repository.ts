...
export class RecognitionRecordRepository extends BaseRepository<'recognition_records', RecognitionRecord> {
  ...
  private requireTenantId(): string {
    ...
    if (!tenantId) {
      throw new Error('Tenant context required for recognition record query');
    }
  ...
  async findByWorker(workerId: Uuid): Promise<RecognitionRecord[]> {
    const rows = await this.db.selectFrom(this.tableName).selectAll().where('tenant_id', '=', this.requireTenantId()).where('to_worker_id', '=', workerId.value).execute();
