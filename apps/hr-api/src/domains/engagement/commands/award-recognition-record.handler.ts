...
@CommandHandler('AwardRecognitionRecord')
@Injectable()
export class AwardRecognitionRecordHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { recognitionRecordId: Uuid };
    const ar = await this.repo.findById(payload.recognitionRecordId);
    if (!ar) throw new Error('Recognition record not found');
    ar.award(command.correlationId);
