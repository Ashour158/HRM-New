...
@CommandHandler('ActivateRecognitionProgram')
@Injectable()
export class ActivateRecognitionProgramHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { recognitionProgramId: Uuid };
    const ar = await this.repo.findById(payload.recognitionProgramId);
    if (!ar) throw new Error('Recognition program not found');
