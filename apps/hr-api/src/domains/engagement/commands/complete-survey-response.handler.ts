...
@CommandHandler('CompleteSurveyResponse')
@Injectable()
export class CompleteSurveyResponseHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { surveyResponseId: Uuid; responses: Record<string, unknown> };
    const ar = await this.repo.findById(payload.surveyResponseId);
    if (!ar) throw new Error('Survey response not found');
    ar.complete(payload.responses, command.correlationId);
