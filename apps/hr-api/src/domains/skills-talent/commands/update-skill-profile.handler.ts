...
@CommandHandler('UpdateSkillProfile')
@Injectable()
export class UpdateSkillProfileHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { skillProfileId: Uuid; skills: { skillId: string; proficiency: number }[] };
    const ar = await this.repo.findById(payload.skillProfileId);
    if (!ar) throw new Error('Skill profile not found');
    ar.updateSkills(payload.skills, command.correlationId);
