...
@CommandHandler('UpdateTalentPool')
@Injectable()
export class UpdateTalentPoolHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { talentPoolId: Uuid; criteria: Record<string, unknown> };
    const ar = await this.repo.findById(payload.talentPoolId);
    if (!ar) throw new Error('Talent pool not found');
    ar.update(payload.criteria, command.correlationId);
