...
@CommandHandler('RemoveTalentPoolMember')
@Injectable()
export class RemoveTalentPoolMemberHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { talentPoolId: Uuid; memberId: string };
    const ar = await this.repo.findById(payload.talentPoolId);
    if (!ar) throw new Error('Talent pool not found');
    ar.removeMember(payload.memberId, command.correlationId);
