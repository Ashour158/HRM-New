...
@CommandHandler('UpdateSuccessionReadiness')
@Injectable()
export class UpdateSuccessionReadinessHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { successionPlanId: Uuid; workerId: string; readinessLevel: number };
    const ar = await this.repo.findById(payload.successionPlanId);
    if (!ar) throw new Error('Succession plan not found');
    ar.updateReadiness(payload.workerId, payload.readinessLevel, command.correlationId);
