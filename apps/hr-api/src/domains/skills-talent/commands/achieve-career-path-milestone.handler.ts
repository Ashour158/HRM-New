...
@CommandHandler('AchieveCareerPathMilestone')
@Injectable()
export class AchieveCareerPathMilestoneHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { careerPathId: Uuid; milestoneId: string };
    const ar = await this.repo.findById(payload.careerPathId);
    if (!ar) throw new Error('Career path not found');
    ar.achieveMilestone(payload.milestoneId, command.correlationId);
