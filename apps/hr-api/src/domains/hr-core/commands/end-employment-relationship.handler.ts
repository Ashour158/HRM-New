...
interface EndEmploymentRelationshipPayload {
  relationshipId: Uuid | string;
  endDate: Date;
  reason: string;
}

@CommandHandler('EndEmploymentRelationship')
@Injectable()
export class EndEmploymentRelationshipHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as EndEmploymentRelationshipPayload;
    ...
    relationship.end(payload.endDate, command.correlationId);
