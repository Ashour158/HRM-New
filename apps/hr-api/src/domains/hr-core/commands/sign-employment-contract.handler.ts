...
import type { CommandResult, HrCommandEnvelope, SignEmploymentContractPayload } from '@hcm/command-contracts';
...
@CommandHandler('SignEmploymentContract')
@Injectable()
export class SignEmploymentContractHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as SignEmploymentContractPayload;
    const contract = await this.employmentContractRepo.findById(toUuid(payload.contractId as Uuid | string));
    if (!contract) throw new NotFoundError('Contract not found');

    if (contract.state === 'DRAFT') {
      contract.offer(command.correlationId);
    }
    contract.sign(payload.signedAt, command.correlationId);
