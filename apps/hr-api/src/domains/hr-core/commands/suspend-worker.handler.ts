...
interface SuspendWorkerPayload {
  workerId: Uuid | string;
  reason?: string;
  effectiveDate?: Date;
}

@CommandHandler('SuspendWorker')
@Injectable()
export class SuspendWorkerHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as SuspendWorkerPayload;
    const worker = await this.workerRepo.findByIdForTenant(toUuid(payload.workerId as Uuid | string), command.tenantId);
    if (!worker) throw new NotFoundError('Worker not found');

    worker.suspend(payload.reason ?? command.reason ?? 'Suspended', command.correlationId);
