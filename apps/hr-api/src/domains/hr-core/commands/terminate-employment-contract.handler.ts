...
import type { CommandResult, HrCommandEnvelope, TerminateEmploymentContractPayload } from '@hcm/command-contracts';
...
@CommandHandler('TerminateEmploymentContract')
@Injectable()
export class TerminateEmploymentContractHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as TerminateEmploymentContractPayload;
    const contract = await this.employmentContractRepo.findById(toUuid(payload.contractId));
    if (!contract) throw new NotFoundError('Contract not found');

    contract.terminate(payload.reason, command.correlationId);
