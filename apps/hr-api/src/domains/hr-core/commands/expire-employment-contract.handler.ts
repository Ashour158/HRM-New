...
import type { CommandResult, ExpireEmploymentContractPayload, HrCommandEnvelope } from '@hcm/command-contracts';
...
@CommandHandler('ExpireEmploymentContract')
@Injectable()
export class ExpireEmploymentContractHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as ExpireEmploymentContractPayload;
    const contract = await this.employmentContractRepo.findById(toUuid(payload.contractId));
    if (!contract) throw new NotFoundError('Contract not found');

    contract.expire(command.correlationId);
