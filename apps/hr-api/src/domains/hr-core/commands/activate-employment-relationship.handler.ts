...
interface ActivateEmploymentRelationshipPayload {
  relationshipId: Uuid | string;
}

@CommandHandler('ActivateEmploymentRelationship')
@Injectable()
export class ActivateEmploymentRelationshipHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as ActivateEmploymentRelationshipPayload;
    const relationship = await this.employmentRelationshipRepo.findById(toUuid(payload.relationshipId as Uuid | string));
    if (!relationship) throw new NotFoundError('Employment relationship not found');

    relationship.activate(command.correlationId);
