...
import { EndJobAssignmentPayload } from '@hcm/command-contracts';
...
@CommandHandler('EndJobAssignment')
@Injectable()
export class EndJobAssignmentHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as EndJobAssignmentPayload;
    ...
    assignment.end(payload.endDate, command.correlationId);
