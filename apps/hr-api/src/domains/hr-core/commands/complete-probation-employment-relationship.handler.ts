...
interface CompleteProbationEmploymentRelationshipPayload {
  relationshipId: Uuid | string;
}

@CommandHandler('CompleteProbationEmploymentRelationship')
@Injectable()
export class CompleteProbationEmploymentRelationshipHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as CompleteProbationEmploymentRelationshipPayload;
    ...
    relationship.confirm(command.correlationId);
