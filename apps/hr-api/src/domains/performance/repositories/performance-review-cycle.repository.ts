...
@Injectable()
export class PerformanceReviewCycleRepository extends BaseRepository<'performance_review_cycles', PerformanceReviewCycle> {
  protected readonly tableName = 'performance_review_cycles' as const;
  ...
  private toAggregate(row: Record<string, never>): PerformanceReviewCycle {
    return new PerformanceReviewCycle({
      ...
      templateId: row.template_id ? new Uuid(row.template_id) : undefined,
      weightings: row.weightings ?? undefined,
      periods: (row.periods as PerformanceReviewCycle['periods']) ?? [],
  ...
  private toRow(entity: PerformanceReviewCycle): Record<string, unknown> {
    return {
      ...
      template_id: entity.templateId?.value ?? null,
      weightings: entity.weightings ? JSON.stringify(entity.weightings) : null,
      periods: JSON.stringify(entity.periods ?? []),
