...
@CommandHandler('ExtendPerformanceImprovementPlan')
@Injectable()
export class ExtendPerformanceImprovementPlanHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { performanceImprovementPlanId: Uuid; newEndDate: Date };
    ...
    ar.extend(payload.newEndDate, command.correlationId);
