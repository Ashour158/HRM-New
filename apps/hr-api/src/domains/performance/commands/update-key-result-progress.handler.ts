...
@CommandHandler('UpdateKeyResultProgress')
@Injectable()
export class UpdateKeyResultProgressHandler {
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { keyResultId: string; currentValue: number };
    ...
    ar.updateProgress(payload.currentValue, command.correlationId);
    await this.repo.save(ar);
    await this.publisher.publishFromAggregate(ar);
    return {
      success: true,
      data: { keyResultId: ar.id.value, status: ar.status, currentValue: ar.currentValue },
