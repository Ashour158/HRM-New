...
@CommandHandler('FinalizeCalibrationSession')
@Injectable()
export class FinalizeCalibrationSessionHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { calibrationSessionId: Uuid };
    const ar = await this.repo.findById(payload.calibrationSessionId);
    if (!ar) throw new Error('Calibration session not found');
    ar.finalize(command.correlationId);
