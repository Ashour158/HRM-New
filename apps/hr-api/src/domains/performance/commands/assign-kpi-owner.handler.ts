...
@CommandHandler('AssignKpiOwner')
@Injectable()
export class AssignKpiOwnerHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { kpiId: string; ownerId: string };
    const id = new Uuid(payload.kpiId);
    const ar = await this.repo.findById(id);
    if (!ar) throw new NotFoundException('KPI not found');
    ar.assignOwner(new Uuid(payload.ownerId), command.correlationId);
    await this.repo.save(ar);
    await this.publisher.publishFromAggregate(ar);
    return {
      success: true,
      data: { kpiId: ar.id.value, status: ar.status, ownerId: ar.ownerId?.value ?? payload.ownerId },
