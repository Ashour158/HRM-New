...
@CommandHandler('CompleteDevelopmentPlan')
@Injectable()
export class CompleteDevelopmentPlanHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { developmentPlanId: string; outcome?: string };
    ...
    ar.complete(payload.outcome ?? 'Completed', command.correlationId);
