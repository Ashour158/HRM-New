...
@CommandHandler('SubmitManagerReview')
@Injectable()
export class SubmitManagerReviewHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { performanceReviewId: Uuid; content: string };
    const ar = await this.repo.findById(payload.performanceReviewId);
    if (!ar) throw new Error('Performance review not found');
    ar.submitManagerReview(payload.content, command.correlationId);
