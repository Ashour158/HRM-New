...
@CommandHandler('RecordDevelopmentMilestone')
@Injectable()
export class RecordDevelopmentMilestoneHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    ...
    ar.recordMilestone({ objectiveTitle: payload.objectiveTitle, status: payload.status }, command.correlationId);
