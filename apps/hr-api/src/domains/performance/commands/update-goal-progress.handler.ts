...
@CommandHandler('UpdateGoalProgress')
@Injectable()
export class UpdateGoalProgressHandler {
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { goalId: Uuid; currentValue: number };
    const ar = await this.repo.findById(payload.goalId);
    if (!ar) throw new Error('Goal not found');
