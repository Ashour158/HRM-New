...
@CommandHandler('CompletePerformanceImprovementPlan')
@Injectable()
export class CompletePerformanceImprovementPlanHandler {
  constructor(
  ...
  async handle(command: HrCommandEnvelope<unknown>): Promise<CommandResult<unknown>> {
    const payload = command.payload as { performanceImprovementPlanId: Uuid; outcome: string };
    ...
    ar.complete(payload.outcome, command.correlationId);
