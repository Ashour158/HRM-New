...
describe('ManagerReports', () => {
  ...
  it('renders only the Team Reports tier cards from the shared catalog', () => {
    render(
      <MemoryRouter>
        <ManagerReports />
      </MemoryRouter>,
    ...
    expect(screen.getByText('Team Performance Distribution')).toBeInTheDocument();
    expect(screen.queryByText('Time-Off Balance & History')).not.toBeInTheDocument();
  });

  it('opens the leave calendar report scoped to the manager\'s direct reports', async () => {
    render(
      <MemoryRouter>
        <ManagerReports />
      </MemoryRouter>,
