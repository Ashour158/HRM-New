...
function statusClasses(status: ConsoleStatus) {
  if (status === 'live') return 'border-secondary/30 bg-secondary/10 text-primary';
  if (status === 'partial') return 'border-primary/30 bg-primary/10 text-primary';
  if (status === 'attention') return 'border-warning/30 bg-warning/60 text-warning-foreground';
    ...
    <Card className="relative h-full overflow-hidden">
      ...
      <CardContent className="flex h-[calc(100%-9rem)] flex-col gap-4 p-5 pt-0">
        ...
        {control.link ? (
          ...
          <div className="mt-auto rounded-lg border border-border bg-muted p-3 text-sm font-semibold text-muted-foreground">
...
function AdminPanelTile({ tool }: { tool: AdminPanelTool }) {
  ...
  const content = (
    <div className="group relative flex h-full min-h-[132px] flex-col fusion-glass rounded-2xl p-4 transition-all hover:-translate-y-0.5 hover:border-secondary/60 hover:shadow-[0_10px_24px_rgba(31,49,86,0.08)]">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="font-semibold leading-5 text-foreground">{tool.label}</p>
          <p className="mt-1 text-sm leading-5 text-muted-foreground">{tool.description}</p>
        </div>
        {tool.status === 'not-configured' ? (
          <span className="absolute right-3 top-3 rounded-full border border-warning/40 bg-warning/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-warning-foreground">
            Setup
          </span>
        ) : null}
      </div>
      <div className="mt-auto flex items-center justify-between pt-4 text-xs font-semibold uppercase tracking-wide text-primary">
  ...
  return (
    <Link to={tool.path} className="focus:outline-none focus:ring-2 focus:ring-primary/30">
      {content}
    </Link>
  );
}
...
function GuidedJourneyCard({ journey }: { journey: GuidedAdminJourney }) {
  ...
  return (
    <Link to={journey.path} className="group focus:outline-none focus:ring-2 focus:ring-primary/30">
      <div className="flex h-full min-h-[240px] flex-col fusion-glass rounded-2xl p-4 transition-all group-hover:-translate-y-0.5 group-hover:border-secondary/60 group-hover:shadow-[0_10px_24px_rgba(31,49,86,0.08)]">
        ...
        <h4 className="mt-4 font-semibold text-foreground">{journey.title}</h4>
        <p className="mt-2 text-sm leading-5 text-muted-foreground">{journey.description}</p>
        <div className="mt-4 flex-1 space-y-2">
          {journey.signals.map((signal) => (
            <div key={signal} className="flex items-start gap-2 text-xs leading-5 text-muted-foreground">
              <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" />
              <span>{signal}</span>
            </div>
          ))}
        </div>
        <div className="mt-4 flex items-center justify-between border-t border-border pt-3 text-xs font-semibold uppercase tracking-wide text-primary">
...
export function AdminSystemConsole() {
  ...
  const adminPanelTools: AdminPanelTool[] = [
    { label: 'Access Governance', description: 'Users, roles, service accounts, access reviews, sensitive-field access, and duty conflicts.', group: 'Foundation', path: '/admin/system-console/access-governance', icon: UserCog, tone: 'text-warning' },
    { label: 'Single Sign-On', description: 'Tenant OIDC and SAML identity providers, JIT provisioning, role mapping, and encrypted secrets.', group: 'Foundation', path: '/admin/system-console/sso', icon: KeyRound, tone: 'text-primary' },
    { label: 'Company Setup', description: 'Departments, locations, employee IDs, required fields, documents, and policy defaults.', group: 'Foundation', path: '/admin/system-console/settings', icon: Settings, tone: 'text-warning' },
    { label: 'Organization Structure', description: 'Legal entities, org units, departments, managers, and reporting lines.', group: 'Foundation', path: '/admin/organization', icon: Building2, tone: 'text-primary' },
    { label: 'Employee Master Data', description: 'Employee records, digital files, employment lifecycle, and worker status.', group: 'Foundation', path: '/admin/employees', icon: Landmark, tone: 'text-primary' },
    { label: 'Employee Data Rules', description: 'Required employee fields, sensitive data visibility, and protected worker information.', group: 'Foundation', path: '/admin/system-console/settings', icon: DatabaseZap, tone: 'text-primary' },
    { label: 'Documents And Files', description: 'Required documents, expiry rules, evidence, and digital file controls.', group: 'Foundation', path: '/admin/system-console/settings', icon: FolderOpen, tone: 'text-primary' },
    { label: 'Leave Management', description: 'Entitlements, balances, requests, approvals, holidays, and payroll impact.', group: 'Workforce Operations', path: '/admin/leave', icon: Umbrella, tone: 'text-primary' },
    { label: 'Attendance And Time', description: 'Check-in rules, workplace evidence, exceptions, daily ledgers, and payroll handoff.', group: 'Workforce Operations', path: '/admin/attendance', icon: CalendarCheck, tone: 'text-warning' },
    { label: 'Shift Scheduling', description: 'Roster planning, coverage gaps, shift bids, swaps, and fatigue controls.', group: 'Workforce Operations', path: '/admin/modules/workforce-management/operations', icon: Timer, tone: 'text-warning' },
    { label: 'HR Service Delivery', description: 'Cases, tasks, service catalog, SLA rules, and employee support queues.', group: 'Workforce Operations', path: '/admin/modules/service-delivery/operations', icon: ClipboardCheck, tone: 'text-warning' },
    { label: 'Travel And Expenses', description: 'Travel requests, expenses, approvals, and reimbursements.', group: 'Workforce Operations', icon: Plane, tone: 'text-primary', status: 'not-configured' },
    { label: 'Payroll Control', description: 'Cycles, statutory packs, blockers, payroll inputs, payslips, and GL handoff.', group: 'Reward And Talent', path: '/admin/payroll', icon: FileText, tone: 'text-warning' },
    { label: 'Compensation', description: 'Bands, pay changes, bonus cycles, equity, and total reward operations.', group: 'Reward And Talent', path: '/admin/modules/compensation/operations', icon: BadgeDollarSign, tone: 'text-secondary' },
    { label: 'Benefits Administration', description: 'Programs, enrollment, life events, carrier reconciliation, and deductions.', group: 'Reward And Talent', path: '/admin/modules/benefits/operations', icon: Umbrella, tone: 'text-primary' },
    { label: 'Performance Management', description: 'Review cycles, goals, feedback, calibration, action plans, and KPIs.', group: 'Reward And Talent', path: '/admin/performance', icon: Trophy, tone: 'text-warning' },
    { label: 'Learning Management', description: 'Courses, assignments, certifications, content packages, and renewals.', group: 'Reward And Talent', path: '/admin/modules/learning/operations', icon: BookOpen, tone: 'text-primary' },
    { label: 'Onboarding', description: 'Preboarding, joining checklists, provisioning, 30/60/90 plans, and probation.', group: 'Reward And Talent', path: '/admin/onboarding', icon: UserRoundPlus, tone: 'text-warning' },
    { label: 'Employee Engagement', description: 'Surveys, recognition programs, engagement signals, and response controls.', group: 'Reward And Talent', path: '/admin/modules/engagement/operations', icon: Radar, tone: 'text-secondary' },
    { label: 'Policy Center', description: 'Scoped policies, lifecycle approval, simulation, application, and evidence.', group: 'Governance And Insights', path: '/admin/system-console/policies', icon: ShieldCheck, tone: 'text-secondary' },
    { label: 'Approval Paths', description: 'Configure command approvals, delegated reviewers, SLA timers, and escalation routes.', group: 'Governance And Insights', path: '/admin/system-console/approvals', icon: GitBranch, tone: 'text-primary' },
    { label: 'Compliance Center', description: 'Policies, acknowledgements, legal holds, statutory reporting, and evidence.', group: 'Governance And Insights', path: '/admin/compliance', icon: ShieldCheck, tone: 'text-primary' },
    { label: 'Country Policy', description: 'Country packs, validations, simulations, approvals, publish, and rollback.', group: 'Governance And Insights', path: '/admin/country-policy', icon: Landmark, tone: 'text-primary' },
    { label: 'Employee Relations', description: 'Cases, investigations, disciplinary actions, accommodations, and closure.', group: 'Governance And Insights', path: '/admin/modules/employee-relations/operations', icon: Briefcase, tone: 'text-secondary' },
    { label: 'Reporting And Analytics', description: 'Workforce, reward, talent, service, and governance reporting.', group: 'Governance And Insights', path: '/admin/reports', icon: FileText, tone: 'text-primary' },
    { label: 'AI Governance', description: 'AI use cases, model runs, bias tests, risk controls, and human oversight.', group: 'Governance And Insights', path: '/admin/modules/hr-ai-governance/operations', icon: Bot, tone: 'text-primary' },
    { label: 'Marketplace', description: 'Extension marketplace and install governance for future add-on services.', group: 'Governance And Insights', icon: Store, tone: 'text-primary', status: 'not-configured' },
    { label: 'Production Readiness', description: 'Go-live gate across setup, policies, modules, integrations, queues, audit, and release controls.', group: 'Governance And Insights', path: '/admin/system-console/readiness', icon: Code2, tone: 'text-warning' },
    { label: 'System Operations', description: 'Recovery queues, integrations, activity reporting, and data safeguards.', group: 'Governance And Insights', path: '/admin/system-console#system-operations', icon: Code2, tone: 'text-warning' },
    { label: 'Failed Work Queue', description: 'Review failed background work, decide recovery actions, and export operator evidence.', group: 'Governance And Insights', path: '/admin/system-console/dead-letter-events', icon: AlertTriangle, tone: 'text-destructive' },
    { label: 'Automation', description: 'Manage scheduled jobs, tenant run windows, manual runs, and last-run health.', group: 'Governance And Insights', path: '/admin/system-console/automation', icon: Timer, tone: 'text-primary' },
    { label: 'Audit Trail', description: 'Search, filter, export, and inspect tenant audit evidence.', group: 'Governance And Insights', path: '/admin/system-console/audit', icon: Radar, tone: 'text-primary' },
    { label: 'Service Event Map', description: 'Review service event names, ownership, versioning, and consuming teams.', group: 'Governance And Insights', path: '/admin/system-console/event-contracts', icon: GitBranch, tone: 'text-primary' },
  ];
    ...
    <div className="min-h-screen fusion-bg">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-6 px-4 py-6 md:px-6 lg:px-8">
        ...
        <section className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
          <Card className="overflow-hidden border-border bg-white">
            <CardContent className="p-0">
              <div className="border-b border-border bg-foreground px-5 py-5 text-white">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-primary">System Console</p>
                  ...
                  <div className="rounded-lg border border-white/20 bg-white/10 p-3 text-sm">
                    ...
                    <p className="mt-1 text-accent">{primaryRole}</p>
                  </div>
                </div>
              </div>
                ...
                priorityAdminWork.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.title}
                      to={item.path}
                      className="group flex items-start justify-between gap-3 rounded-lg border border-warning/30 bg-warning/10 p-3 transition-colors hover:bg-white focus:outline-none focus:ring-2 focus:ring-warning/20"
                    >
                      <div className="flex min-w-0 items-start gap-3">
                        <div className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-white text-warning">
                          <Icon className="h-4 w-4" />
                        </div>
                        <div className="min-w-0">
                          <p className="font-semibold text-foreground">{item.title}</p>
                          <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{item.helper}</p>
                        </div>
                      </div>
                      <ArrowRight className="mt-2 h-4 w-4 shrink-0 text-primary transition-transform group-hover:translate-x-1" />
                    </Link>
                  );
                })
              ) : (
                <div className="rounded-lg border border-secondary/20 bg-secondary/10 p-4">
                  <p className="font-semibold text-foreground">No urgent admin work is waiting.</p>
                  <p className="mt-1 text-sm leading-6 text-muted-foreground">Use the guided work paths below to review setup, policies, access, integrations, and health.</p>
                </div>
              )}
        ...
        <section className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
          <Card className="relative overflow-hidden border-border bg-white">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm leading-5 text-muted-foreground">{group.helper}</p>
                        </div>
                      </div>
                      ...
                      {group.items.map((item) => {
                        ...
                        return (
                          <Link
                            key={item.title}
                            to={item.path}
                            className="group block rounded-xl border border-border bg-white p-3 transition-all hover:-translate-y-0.5 hover:border-warning/50 hover:shadow-[0_10px_24px_rgba(31,49,86,0.08)] focus:outline-none focus:ring-2 focus:ring-warning/20"
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex min-w-0 items-start gap-3">
                                <div className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-warning/10 text-warning">
                                  <Icon className="h-4 w-4" />
                                </div>
                                <div className="min-w-0">
                                  <p className="font-semibold text-foreground">{item.title}</p>
                              ...
                              <div className="shrink-0 text-right">
                                <p className="font-headline text-2xl font-bold text-foreground">{item.count}</p>
                                <StatusBadge status={item.status} label={item.statusLabel} />
                              </div>
                            </div>
                            <div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-xs font-semibold uppercase tracking-wide text-primary">
        ...
        <section id="guided-work-paths" className="space-y-4 scroll-mt-28">
          <div>
            <h3 className="font-headline text-xl font-bold text-foreground">Guided Work Paths</h3>
        ...
        <section className="space-y-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h3 className="font-headline text-xl font-bold text-foreground">Administration Areas</h3>
              <p className="mt-1 text-sm text-muted-foreground">Grouped by how HR administrators actually manage the platform.</p>
            </div>
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="h-9 w-64 rounded-lg border border-border bg-white pl-9 pr-3 text-sm outline-none transition-colors placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary/10"
          ...
          <div className="space-y-5">
            {adminPanelGroups.map((group) => {
              ...
              return (
                <Card key={group} className="border-border bg-muted">
        ...
        <section className="space-y-5">
          <h3 className="font-headline text-xl font-bold text-foreground">Custom Services</h3>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            <AdminPanelTile
              tool={{
                label: 'Add Service',
            ...
            return (
              <Card key={metric.label} className="relative overflow-hidden">
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="font-mono text-xs uppercase tracking-wider text-muted-foreground">{metric.label}</p>
        ...
        <section id="system-operations" className="grid gap-4 scroll-mt-28 xl:grid-cols-[1fr_24rem]">
          ...
          <div className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-3 text-sm leading-6 text-muted-foreground">
                <div className="rounded-lg border border-border bg-muted p-3">
                  <p className="font-semibold text-foreground">Sign-in mode</p>
                  <p>{authBypassEnabled ? 'Local sign-in shortcut is enabled for this build.' : 'Standard sign-in is enforced for this build.'}</p>
                </div>
                <div className="rounded-lg border border-border bg-muted p-3">
                  <p className="font-semibold text-foreground">Live policy changes</p>
                  <p>Only applied policy revisions change live service behavior.</p>
                </div>
                <div className="rounded-lg border border-border bg-muted p-3">
                  <p className="font-semibold text-foreground">Historical data</p>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm leading-6 text-muted-foreground">
                <p className="rounded-lg border border-warning/30 bg-warning/40 p-3 text-warning-foreground">
          ...
          <Card className="relative overflow-hidden">
            ...
            <CardContent className="space-y-3 p-5 pt-0">
              {notificationsQuery.isLoading ? (
                ...
                <p className="fusion-glass rounded-2xl p-4 text-sm text-muted-foreground">
              ...
              {auditQuery.isLoading ? (
                ...
                (auditQuery.data ?? []).slice(0, 5).map((record) => (
                  <div key={record.id} className="fusion-glass rounded-2xl p-3">
                    <div className="flex items-start justify-between gap-3">
                      <p className="text-sm font-semibold text-foreground">{record.action}</p>
            ...
            {commercialModules.map((module) => {
              ...
              return (
                <Link key={module.id} to={target} className="group">
                  <div className="flex h-full min-h-[150px] flex-col fusion-glass rounded-2xl p-4 transition-all group-hover:-translate-y-0.5 group-hover:border-secondary/70 group-hover:bg-muted">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-semibold text-foreground">{module.label}</p>
                        <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{module.category}</p>
                      </div>
                      <StatusBadge status={status} label={moduleMaturityLabel(module)} />
                    </div>
                    <p className="mt-3 line-clamp-3 text-sm leading-6 text-muted-foreground">{module.summary}</p>
                    <div className="mt-auto flex items-center justify-between pt-3 text-sm font-semibold text-primary">
