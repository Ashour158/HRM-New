...
export function AdminPayroll() {
  ...
  return (
    <div className="-m-4 min-h-[calc(100vh-7rem)] bg-slate-100">
      <div className="bg-[#25345f] px-6 py-5 text-white shadow-sm">
        ...
        <div>
          ...
          <p className="text-sm text-white/75">Monthly payroll cycles fed by attendance, compensation, deductions, and tenant policies.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button className="bg-[#0b7cff] text-white hover:bg-[#096ee0]" onClick={() => closeToPay(false)} disabled={closeToPayMutation.isPending || previewLoading || rows.length === 0}>
      ...
      <Tabs value={activePayrollTab} onValueChange={(value) => setActivePayrollTab(value as PayrollTab)}>
        <div className="sticky top-0 z-10 border-b bg-white px-6 py-2">
          <TabsList className="h-auto flex-wrap justify-start bg-transparent p-0">
            {payrollTabs.map((tab) => (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="rounded-none border-b-2 border-transparent px-4 py-3 data-[state=active]:border-[#0b7cff] data-[state=active]:bg-transparent data-[state=active]:text-[#0b7cff] data-[state=active]:shadow-none"
              >
                {tab.label}
              </TabsTrigger>
            ))}
      ...
      <section id="payroll-cycle" className={tabClass('cycle', 'grid gap-4 border-b bg-white py-6 md:grid-cols-4')}>
      ...
      <section className={tabClass('cycle', 'grid gap-4 border-b bg-white py-6 md:grid-cols-5')}>
        <div>
