          ...
          <>
            <section className="grid gap-4 md:grid-cols-3">
              <Card className="rounded-lg border-[#bbcabf]">
                <CardHeader className="pb-2">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Network className="h-5 w-5 text-[#006c49]" />
                    Topics
                  </CardTitle>
              ...
              <Card className="rounded-lg border-[#bbcabf]">
                <CardHeader className="pb-2">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <GitBranch className="h-5 w-5 text-[#4648d4]" />
                    Aggregate Routes
                  </CardTitle>
              ...
              <Card className="rounded-lg border-[#bbcabf]">
                ...
                <CardContent className="space-y-1 text-sm text-[#3c4a42]">
                  <div>Topic: <span className="font-mono text-[#0b1c30]">{registryQuery.data?.defaults.topic}</span></div>
            ...
            <Card className="rounded-lg border-[#bbcabf]">
              ...
              <CardContent className="grid gap-3 text-sm md:grid-cols-2">
                <div className="rounded-lg bg-[#eff4ff] p-4">
                  <div className="text-xs font-semibold uppercase tracking-wide text-[#6c7a71]">Convention</div>
                  <div className="mt-1 font-mono">{registryQuery.data?.consumerGroupNaming.convention}</div>
                </div>
                <div className="rounded-lg bg-[#eff4ff] p-4">
                  <div className="text-xs font-semibold uppercase tracking-wide text-[#6c7a71]">Example</div>
            ...
            <Card className="overflow-hidden rounded-lg border-[#bbcabf]">
              <CardHeader className="gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="relative w-full md:w-80">
                  ...
                  <input className="w-full rounded-lg border border-[#bbcabf] bg-white py-2 pl-9 pr-3 text-sm" placeholder="Search aggregate or topic" value={search} onChange={(event) => setSearch(event.target.value)} />
                </div>
              </CardHeader>
                    ...
                    <TableBody>
                      {aggregateMappings.map((row) => (
                        <TableRow key={row.aggregateType}>
                          ...
                          <TableCell className="font-mono text-sm text-[#3c4a42]">{row.topic}</TableCell>
                        </TableRow>
                      ))}
                      {!aggregateMappings.length && (
                        <TableRow>
                          <TableCell colSpan={2} className="py-10 text-center text-[#6c7a71]">No event mappings match this search.</TableCell>
                        </TableRow>
                      )}
                    </TableBody>
                  </Table>
            ...
            <Card className="rounded-lg border-[#bbcabf]">
              ...
              <CardContent className="grid gap-2 md:grid-cols-2">
                {(registryQuery.data?.eventPrefixMappings ?? []).map(([prefix, topic]) => (
                  <div key={`${prefix}-${topic}`} className="rounded-lg border border-[#bbcabf] bg-white p-3 text-sm">
                    <span className="font-mono font-semibold">{prefix}</span>
                    <span className="mx-2 text-[#6c7a71]">{'->'}</span>
                    <span className="font-mono text-[#3c4a42]">{topic}</span>
                  </div>
                ))}
              </CardContent>
            </Card>
