...
const weatherThemes = {
  ...
  "partly-cloudy": {
    label: "Partly cloudy",
    Icon: CloudSun,
    gradient: "from-orange-300 via-rose-400 to-amber-500",
  ...
  snowy: {
    label: "Snowy",
    Icon: CloudSnow,
    gradient: "from-teal-200 via-cyan-300 to-slate-400",
    glow: "bg-teal-200/50",
  },
} as const;
...
const moduleIcon: Record<string, string> = {
  ...
  indigo: "bg-amber-100 text-amber-600",
  violet: "bg-rose-100 text-rose-600",
  purple: "bg-pink-100 text-pink-600",
  fuchsia: "bg-teal-100 text-teal-600",
  rose: "bg-orange-100 text-orange-600",
};
...
const alertBg: Record<string, string> = {
  high: "bg-rose-50/70 hover:bg-rose-100/70",
  medium: "bg-amber-50/70 hover:bg-amber-100/70",
  low: "bg-teal-50/70 hover:bg-teal-100/70",
};

const activityTag: Record<string, string> = {
  blue: "bg-orange-100 text-orange-700",
  indigo: "bg-amber-100 text-amber-700",
  violet: "bg-rose-100 text-rose-700",
  fuchsia: "bg-pink-100 text-pink-700",
  rose: "bg-teal-100 text-teal-700",
};
const activityIcon: Record<string, string> = {
  blue: "text-orange-500",
  indigo: "text-amber-500",
  violet: "text-rose-500",
      ...
      <div className="flex items-center gap-3">
        {!isActive ? (
          <button
            ...
            className="flex items-center gap-2 px-7 py-3.5 rounded-2xl font-bold text-white bg-gradient-to-r from-orange-500 to-rose-500 shadow-lg shadow-orange-500/25 hover:shadow-xl hover:shadow-orange-500/30 transition-all active:scale-95"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-7 py-3.5 rounded-2xl font-bold text-white bg-gradient-to-r from-rose-500 to-pink-600 shadow-lg shadow-rose-500/25 hover:shadow-xl hover:shadow-rose-500/30 transition-all active:scale-95"
          >
    ...
    <div className="min-h-screen fusion-bg text-slate-900 font-['Plus_Jakarta_Sans',sans-serif] flex relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0" aria-hidden="true">
        <div
          ...
          style={{ width: 420, height: 420, top: -80, left: "30%", background: "radial-gradient(circle, #fdba74, #fb923c)" }}
        />
        <div
          className="fusion-blob fusion-blob-2"
          style={{ width: 360, height: 360, top: "20%", right: "-60px", background: "radial-gradient(circle, #fda4af, #fb7185)" }}
        />
        <div
          className="fusion-blob fusion-blob-3"
          style={{ width: 380, height: 380, bottom: "-100px", left: "20%", background: "radial-gradient(circle, #fcd34d, #2dd4bf)" }}
        />
      </div>
      ...
      <aside className="w-64 fusion-glass border-r border-white/40 flex-col z-10 hidden lg:flex relative">
              ...
              {[
                ...
                <a
                  ...
                  className={`flex items-center px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors ${
                    item.active
                      ? "bg-white/80 text-orange-700 shadow-sm"
                      : "text-slate-600 hover:bg-white/50 hover:text-slate-900"
                  }`}
                >
                  <item.icon
                    size={18}
                    className={`mr-3 ${item.active ? "text-orange-600" : "text-slate-400"}`}
                  />
                  {item.label}
                </a>
              ))}
        ...
        <div className="p-4 border-t border-white/30">
          <div className="flex items-center px-2">
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-orange-400 to-rose-400 flex items-center justify-center text-white font-bold text-sm border-2 border-white shadow-sm">
              JD
            </div>
        ...
        <header className="h-16 fusion-glass border-b border-white/30 flex items-center justify-between px-6 lg:px-8 z-20">
          ...
          <div className="hidden lg:flex items-center max-w-md w-full relative">
            ...
            <input
              ...
              className="w-full bg-white/55 border border-white/60 focus:bg-white/80 focus:ring-2 focus:ring-orange-500/20 focus:outline-none rounded-full pl-10 pr-4 py-2 text-sm placeholder:text-slate-400 transition-all shadow-inner"
            />
          </div>
          <div className="flex items-center space-x-3">
            ...
            >
              <img
                ...
                className="w-10 h-10 rounded-full object-cover ring-2 ring-white/70 shadow-md group-hover:ring-orange-400 group-hover:scale-105 transition-all"
              />
        ...
        <div className="flex-1 p-6 lg:p-8">
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col lg:flex-row lg:items-stretch justify-between gap-6">
              <div className="flex flex-col justify-center">
                ...
                <p className="text-slate-600 max-w-2xl text-base md:text-lg font-medium leading-relaxed flex items-start gap-2.5">
                  <Quote
                    size={20}
                    className="text-orange-400 shrink-0 mt-1 -scale-x-100"
                    fill="currentColor"
                  />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              {/* KPI 1 */}
              <div className="bg-gradient-to-br from-orange-400 to-rose-500 text-white rounded-[2rem] p-6 fusion-hover flex flex-col justify-between h-[180px] shadow-lg shadow-orange-500/20 relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start relative z-10">
                  ...
                  <ArrowRight size={20} className="text-orange-100/80" />
                </div>
                <div className="relative z-10">
                  <div className="text-4xl font-extrabold mb-1">4,820</div>
                  <div className="text-orange-50/90 text-sm font-medium">
                    Total Headcount · Current active
                  </div>
                </div>
              </div>
              {/* KPI 2 */}
              <div className="bg-gradient-to-br from-amber-400 to-orange-500 text-white rounded-[2rem] p-6 fusion-hover flex flex-col justify-between h-[180px] shadow-lg shadow-orange-500/20 relative overflow-hidden group">
                <div className="absolute -right-6 -top-6 w-32 h-32 bg-white/15 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500" />
                <div className="flex justify-between items-start relative z-10">
                  <div className="p-3 bg-white/20 rounded-2xl backdrop-blur-sm">
                    <TrendingDown size={24} />
                  </div>
                  <div className="px-2.5 py-1 bg-orange-900/25 rounded-full text-xs font-bold backdrop-blur-sm">
                    -1.2%
                  </div>
                </div>
                <div className="relative z-10">
              ...
              {/* KPI 3 */}
              <div className="bg-gradient-to-br from-rose-400 to-pink-500 text-white rounded-[2rem] p-6 fusion-hover flex flex-col justify-between h-[180px] shadow-lg shadow-pink-500/20 relative overflow-hidden group">
                <div className="absolute -right-6 -top-6 w-32 h-32 bg-white/15 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500" />
                <div className="flex justify-between items-start relative z-10">
                  <div className="p-3 bg-white/20 rounded-2xl backdrop-blur-sm">
                    <Briefcase size={24} />
                  </div>
                  <ArrowRight size={20} className="text-rose-100/80" />
                </div>
                <div className="relative z-10">
                  <div className="text-4xl font-extrabold mb-1">137</div>
                  <div className="text-rose-50/90 text-sm font-medium">
                    Open Positions · Demand
                  </div>
                </div>
              </div>
              {/* KPI 4 */}
              <div className="bg-gradient-to-br from-teal-400 to-cyan-500 text-white rounded-[2rem] p-6 fusion-hover flex flex-col justify-between h-[180px] shadow-lg shadow-teal-500/20 relative overflow-hidden group">
                <div className="absolute -right-6 -top-6 w-32 h-32 bg-white/15 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500" />
                <div className="flex justify-between items-start relative z-10">
                  <div className="p-3 bg-white/20 rounded-2xl backdrop-blur-sm">
                    <UserPlus size={24} />
              ...
              <div className="lg:col-span-3 fusion-glass rounded-[2rem] p-6 lg:p-8 flex flex-col">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-4 text-xs font-semibold text-slate-500">
                    <span className="flex items-center gap-1.5">
                      <span className="w-3 h-3 rounded-full bg-orange-500" /> Hires
                    </span>
                    <span className="flex items-center gap-1.5">
                      <span className="w-3 h-3 rounded-full bg-teal-400" /> Exits
                    </span>
                  </div>
                </div>
                <div className="h-72 -ml-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-6 flex flex-col">
                ...
                <div className="fusion-glass rounded-[2rem] p-6 flex flex-col relative overflow-hidden flex-1">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-orange-400/15 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />
