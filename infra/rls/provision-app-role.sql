...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'hcm_app') THEN
    EXECUTE format('CREATE ROLE hcm_app LOGIN PASSWORD %L', :app_password);
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'hcm_system') THEN
    EXECUTE format('CREATE ROLE hcm_system LOGIN BYPASSRLS PASSWORD %L', :app_password);
  END IF;
END $$;
