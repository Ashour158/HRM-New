 ...
 * `StatutoryLeaveTypeRepository` (tenant-isolation hardening follow-up to
 * PR #117), and `WorksCouncilConsultationRepository` in the `global-hr`
 * domain — see each repository file for the `executor` getter / inherited
 * `BaseRepository` usage. All other repositories in the codebase (~120+
 * across the remaining domains) still construct their own pooled `Kysely`
 * instance and write outside the command's transaction; adopting this
 * pattern for them is intentionally out of scope here (a repo-wide rollout
 * needs its own staged effort, not a drive-by change bundled with an
 * unrelated fix). Do not assume any repository outside the ones named above
 * is transaction-safe.
 */
